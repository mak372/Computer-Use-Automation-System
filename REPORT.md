...
Only two parts of the system are browser-specific: one reads the page e.g. asking the browser for its accessibility tree, the buttons, text fields, and labels currently visible  and one acts on it e.g. clicking a button or typing into a field through the browser automation library. Decision-making, safety checks, outcome verification, human handoffs, and logging works with the information those parts provide, regardless of the underlying surface. Supporting a desktop or legacy application would therefore mainly require replacing those two parts rather than rebuilding the core pipeline. The same principle carries over to different tenants, but what changes isn't a piece of code, it's which artifact gets used — a different institution's workflow becomes a different capability_version of the same capability, rather than a special code path. The system doesn't automatically detect or adapt to differences
between tenants' workflows; that judgment happens during human artifact review, and gets recorded as a separate `capability_version`. A caller can optionally pin the `capability_version` it expects; if it does, replay refuses to run against a different version rather than silently assuming compatibility. A caller that doesn't pin one just gets whatever version the artifact currently is.
...
- **Web-specific drift detection:** `expected_target` records an HTTP method and route pattern to catch an element that still looks right but now leads somewhere different. A desktop or canvas-only surface has no HTTP method or route to record. Extending to those surfaces would mean recording whatever that surface's real equivalent of "destination" is (e.g. which window or view an action opens), not reusing this representation as-is.